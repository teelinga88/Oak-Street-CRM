...
export default function CRM() {
  ...
  async function handleDeleteAccount() {
    ...
    setModal(null); setSelId(null); showToast('Account deleted');
  }
  ...
  function openDealModal(id=null) {
    ...
    setDealCompany(d?.account || '');
    setIsNewProspect(false);
    setCompanySuggs([]);
    setModal({ type:'deal', id });
  }
  ...
  async function handleDeleteDeal() {
    ...
    await deleteDeal(modal.id);
    setModal(null); setSelId(null); showToast('Prospect deleted');
  }
  ...
  async function saveFollowup() {
    ...
    if (modal.id) {
      await updateFollowup(modal.id, data);
    } else {
  ...
  async function handleDeleteFollowup() {
    ...
    await deleteFollowup(modal.id);
    setModal(null); setSelId(null); showToast('Follow-up deleted');
  }

  // ── Log activity ──
  const [logState, setLogState] = useState({ type:'Call', text:'' });
  function openLogModal(accountId, followupId=null) {
    setLogState({ accountId, followupId, type:'Call', text:'' });
    setModal({ type:'log' });
  }
  async function saveLog() {
    if (!logState.text?.trim()) { showToast('Description required'); return; }
    const a = accounts.find(x => x.id === logState.accountId);
    if (!a) return;
    const newActivity = { text:`[${logState.type}] ${logState.text.trim()}`, time:nowLabel() };
    const activities = [newActivity, ...(a.activities||[])];
    await updateAccount(a.id, { activities, lastContact: new Date().toISOString() });
    if (logState.followupId) {
      await updateFollowup(logState.followupId, { done: true });
    }
  ...
  function openBucketForm() {
    setBucketForm({ company:'', contact:'', email:'', phone:'', location:'' });
    setModal({ type:'addLead' });
  }
  async function saveLead() {
    if (!bucketForm.company?.trim()) { showToast('Company name required'); return; }
    await addLead({ ...bucketForm, rep: repName });
    ...
    <div style={{ display:'flex', height:'100vh', fontFamily:'-apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif', fontSize:14, color:'#1a1a1a', background:'#fff' }}>
      ...
      <div style={{ width:210, borderRight:'0.5px solid #E5E4DF', background:'#F7F6F3', display:'flex', flexDirection:'column', flexShrink:0 }}>
        <div style={{ padding:'16px 16px 12px', borderBottom:'0.5px solid #E5E4DF' }}>
          <div style={{ display:'flex', alignItems:'center', gap:8 }}>
            ...
            <div>
              <div style={{ fontWeight:600, fontSize:13 }}>Oak Street Logistics</div>
              <div style={{ fontSize:11, color:'#888' }}>Sales CRM</div>
            </div>
          </div>
        </div>
        ...
        <div style={{ padding:'10px 14px', borderBottom:'0.5px solid #E5E4DF', display:'flex', alignItems:'center', gap:8 }}>
          <div style={{ width:28, height:28, borderRadius:'50%', display:'flex', alignItems:'center', justifyContent:'center', fontSize:10, fontWeight:600, background:repProfile?.color[0], color:repProfile?.color[1], flexShrink:0 }}>
            {repProfile?.initials}
          </div>
          <div>
            <div style={{ fontSize:12, fontWeight:500 }}>{repProfile?.name}</div>
            <div style={{ fontSize:10, color:'#888' }}>{isManager ? 'Rep & Manager' : 'Sales Rep'}</div>
          </div>
        </div>
        ...
        <div style={{ padding:'10px 14px', borderTop:'0.5px solid #E5E4DF' }}>
          <button onClick={logout} style={{ ...S.btn, width:'100%', justifyContent:'center', fontSize:12, color:'#888' }}>
            Sign out
          </button>
        </div>
      </div>
            ...
            <div style={{ padding:'12px 16px', borderBottom:'0.5px solid #E5E4DF', display:'flex', alignItems:'center', justifyContent:'space-between', flexShrink:0 }}>
              ...
              <div style={{ display:'flex', gap:8 }}>
                <select value={statusFilter} onChange={e=>setStatusFilter(e.target.value)} style={{ ...S.input, width:'auto', padding:'5px 10px' }}>
                  ...
                  {ACCT_STATUSES.map(s=><option key={s}>{s}</option>)}
                </select>
            ...
            <div style={{ flex:1, overflowY:'auto', padding:16 }}>
              ...
              {leads.length === 0 ? (
                <div style={{ textAlign:'center', padding:40, color:'#888' }}>
                  ...
                  Bucket empty. Add leads to get started.
                </div>
              ) : (
                <div style={{ display:'grid', gridTemplateColumns:'repeat(2,1fr)', gap:12 }}>
                  {leads.map(lead => (
                    <div key={lead.id} style={S.card}>
                      ...
                      {lead.notes?.length > 0 && (
                        <div style={{ fontSize:11, color:'#aaa', background:'#fff', borderRadius:6, padding:'5px 8px', marginBottom:8 }}>
                          {lead.notes[0].text} · {lead.notes[0].time}
                        </div>
                      )}
                      <div style={{ display:'flex', gap:6 }}>
                        <button style={{ ...S.btn, flex:1, justifyContent:'center', fontSize:11 }} onClick={()=>{
                          ...
                          updateLead(lead.id, { attempts:(lead.attempts||0)+1, notes });
                        ...
                        <button style={{ ...S.btnPrimary, flex:1, justifyContent:'center', fontSize:11 }} onClick={async ()=>{
                          ...
                          if (!acct) {
                            const ref = await addAccount({ name:lead.company, industry:'', location:lead.location||'', status:'Active', contact:lead.contact||'', email:lead.email||'', phone:lead.phone||'', rep:repName, shipmentType:'', commodity:'', notes:'', activities:[{text:`[Call] ${note}`, time:nowLabel()}], lastContact:new Date().toISOString() });
                            acct = { id: ref.id, name: lead.company };
                          }
                  ...
                  <div style={{ border:'0.5px solid #E5E4DF', borderRadius:10, padding:12, marginBottom:8, background:'#fff' }}>
                    ...
                    {!f.done && f.accountId && (
                      <div style={{ marginTop:8 }}>
                        <button style={{ ...S.btnLog, width:'100%', justifyContent:'center', fontSize:11 }} onClick={()=>openLogModal(f.accountId, f.id)}>
                          ✏️ Log what happened
                        </button>
                      </div>
                    )}
                    ...
                    return (
                      <div key={email} onClick={()=>setMgrSel({ type:'rep', value:email })}
                        ...
                        <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                          ...
                          <div>
                            <div style={{ fontSize:12, fontWeight:500 }}>{rep.name.split(' ')[0]}</div>
                            {rep.isManager && <div style={{ fontSize:10, color:'#aaa' }}>Mgr</div>}
                          </div>
                        </div>
      ...
      <div style={{ width:250, borderLeft:'0.5px solid #E5E4DF', display:'flex', flexDirection:'column', flexShrink:0, overflow:'hidden' }}>
        <div style={{ padding:'12px 14px', borderBottom:'0.5px solid #E5E4DF', display:'flex', alignItems:'center', justifyContent:'space-between', flexShrink:0 }}>
          ...
          {selId && (view==='accounts'||view==='pipeline') && (
            <button style={{ ...S.btn, padding:'4px 10px', fontSize:11 }} onClick={()=>{ if(view==='accounts') openAccountModal(selId); else openDealModal(selId); }}>✏️ Edit</button>
          )}
        </div>
        <div style={{ flex:1, overflowY:'auto', padding:14 }}>
                ...
                {a.activities?.length > 0 && <DetailSection title="Activity log">
                  {a.activities.map((act,i) => (
                    <div key={i} style={{ display:'flex', gap:8, marginBottom:10 }}>
                      ...
                      <div>
                        <div style={{ fontSize:12, color:'#888', lineHeight:1.5 }}>{act.text}</div>
                        <div style={{ fontSize:11, color:'#aaa', marginTop:2 }}>{act.time}</div>
                      </div>
                    </div>
                  ))}
                </DetailSection>}
            ...
            return (
              <>
                ...
                <DetailSection title="Notes & activity">
                  ...
                  )) : <div style={{ fontSize:12, color:'#aaa', padding:'6px 0' }}>No notes yet — click Edit to add</div>}
                </DetailSection>
              </>
            );
          })()}
          ...
          {view==='manager' && mgrSel && (() => {
            if (mgrSel.type === 'rep') {
              const rep = TEAM_ROSTER[mgrSel.value];
              if (!rep) return null;
              const repDeals = deals.filter(d=>d.rep===rep.name);
              const repAccts = accounts.filter(a=>a.rep===rep.name);
              const repFu = followups.filter(f=>f.rep===rep.name&&!f.done);
          ...
          {/* Empty state */}
          {!selId && !mgrSel && (
            <div style={{ textAlign:'center', padding:40, color:'#888', lineHeight:1.8 }}>
      ...
      {modal?.type === 'account' && (
        <Modal title={modal.id ? 'Edit account' : 'New account'} onClose={()=>setModal(null)} onSave={saveAccount} showDelete={!!modal.id} onDelete={handleDeleteAccount}>
          <FGrid><FRow label="Company name *"><input style={S.input} value={af.name||''} onChange={e=>setAf({...af,name:e.target.value})} placeholder="Company Name"/></FRow>
          <FRow label="Industry"><input style={S.input} value={af.industry||''} onChange={e=>setAf({...af,industry:e.target.value})} placeholder="Manufacturing"/></FRow></FGrid>
          <FGrid><FRow label="City, State"><input style={S.input} value={af.location||''} onChange={e=>setAf({...af,location:e.target.value})} placeholder="Chicago, IL"/></FRow>
          <FRow label="Status"><select style={S.input} value={af.status||'Active'} onChange={e=>setAf({...af,status:e.target.value})}>{ACCT_STATUSES.map(s=><option key={s}>{s}</option>)}</select></FRow></FGrid>
      ...
      {modal?.type === 'log' && (
        <Modal title="Log activity" sub={logState.followupId ? 'Logging follow-up activity' : 'Log activity'} onClose={()=>setModal(null)} onSave={saveLog} saveLabel="Log activity">
          <FRow label="Activity type">
            <select style={S.input} value={logState.type} onChange={e=>setLogState({...logState,type:e.target.value})}>
              {ACT_TYPES.map(t=><option key={t}>{t}</option>)}
            </select>
          </FRow>
          <FRow label="Description *">
            <textarea style={{ ...S.input, minHeight:80, resize:'vertical' }} value={logState.text} onChange={e=>setLogState({...logState,text:e.target.value})} placeholder="e.g. Emailed John about FTL rates — he's reviewing with his team…" autoFocus/>
          </FRow>
      ...
      {/* Toast */}
      {toast && (
        <div style={{ position:'fixed', bottom:16, left:'50%', transform:'translateX(-50%)', background:'#1a1a1a', color:'#fff', padding:'8px 16px', borderRadius:8, fontSize:12, zIndex:200, whiteSpace:'nowrap' }}>
          {toast}
        </div>
      )}
    </div>
...
function DetailSection({ title, children }) {
  return (
    <div style={{ marginBottom:14 }}>
      <div style={{ fontSize:10, fontWeight:500, color:'#aaa', textTransform:'uppercase', letterSpacing:'.05em', marginBottom:6 }}>{title}</div>
      {children}
    </div>
  );
}
function DetailRow({ k, v }) {
  return (
    <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', fontSize:12, padding:'4px 0', borderBottom:'0.5px solid #F0EFE8' }}>
      <span style={{ color:'#888' }}>{k}</span>
      <span style={{ fontWeight:500, textAlign:'right', maxWidth:150, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{v}</span>
    </div>
  );
}
